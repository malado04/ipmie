...
$stmt = $pdo->prepare("SELECT * FROM enfants WHERE idEnfants=:idEnfants");
$stmt->execute(array(":idEnfants"=>$_GET['id']));
$userRow=$stmt->fetch(PDO::FETCH_ASSOC);
echo($userRow['idEnfants']);
...
<!-- Ajouter un utilisateur -->
  ...
  <div class="modal-dialog modal-lg" style="max-width: 1800px!important;"  role="document">
    <div class="modal-content">
      <div class="modal-header navbar orange row"> 
           <h3 class="labelh">Modifier un nouveau enfant </h3>       
  ...
  <div class="modal-body">         
    <form action="update_db_e_en.php" method="post" class="row">   
    <div class="col-md-12"><!-- 
    <label>Photo de l'enfant</label>
    <input type="file" name="image" id="image" class="form-control"  placeholder="Photo de l'enfant" required /> <br/>  -->
    <label>Nom de l'enfant</label>
    <input type="text" name="nomenfant" id="nomenfant" class="form-control"  placeholder="Nom de l'enfant" required  value="<?php echo $userRow['nomEnfants']; ?>"/> <br/> 
    <label>Prénom de l'enfant</label>
    <input type="text" name="prenomenfant" id="prenomenfant" class="form-control"  placeholder="Prénom de l'enfant" required  value="<?php echo $userRow['prenomEnfants']; ?>"/><br/> 
    <label>Date de naissance de l'enfant</label>
    <input type="date" name="datenaissanceenfant" id="datenaissanceenfant" class="form-control"  placeholder="Date de naisasnce de l'enfant" required  value="<?php echo $userRow['dateNaissanceEnfants']; ?>"/> <br/> 
    <select name="sex"  class="form-control" value="<?php echo $userRow['sex']; ?>">
      <option value="M">Masculin</option>
      <option value="F">Feminin</option>
    </select><br>
    <label>Lieu de naisance de l'enfant</label>
    <input type="text" name="lieunaissanceenfant" id="lieunaissanceenfant" class="form-control"  placeholder="Lieu de naissance  de l'enfant" required  value="<?php echo $userRow['LieuNaissanceEnfants']; ?>"/> <br/> 
    <label>Numéro régistre de l'enfant</label>
    <input type="number" name="numeroregistre" id="numeroregistre" class="form-control"  placeholder="Numéro régistre de l'enfant" required  value="<?php echo $userRow['numeroRegistreEnfants']; ?>"/>
  </div>  
  </div>  
    <div class="modal-footer row">
            ...
            <div class="col-md-3">
              <input type="hidden" name="idEmployer" value="<?php echo $_GET['id']; ?>">
              <input type="hidden" name="ajoutEnfant" value="ajoutEnfant" >
