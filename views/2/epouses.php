...
<h2 class="text-center">Liste des epouses</h2><br><br>
        <!-- Begin Page Content -->
        <div class="container-fluid">
        ...
        <table id="paiement" class="table table-hover table-striped table-bordered" style="width:100%">
          <thead>
            <tr>
              ...
              <th>Photo epouses</th>
              <th>Nom epouses</th>
              <th>Prénom epouses</th>
              <th>Date de naissance</th> 
              <th>Lieu de naissance</th> 
              <th>Numéro de registre</th>
              <th>Ajouter photo !</th>   
              <th>Modifier époux (se)!</th>   
            </tr>
          </thead>
          <tbody>
            <?php  
            ...
            $statement = $pdo->prepare("SELECT * FROM epouses ORDER BY idEpouses DESC");  
            // $statement = $pdo->prepare("SELECT * FROM epouses, employer WHERE enfants.employer_id =employer.idEmployer ORDER BY idEnfants DESC");  
            $statement->execute(array());  
            $result = $statement->fetchAll();
            foreach ($result as $row) { 
              echo '<tr>';
                echo '<td>' . $row['idEpouses'] . '</td>';
                echo '<td><img  class="width_img img-profile" src="../../img/epouse/'.$row['image'].'"></td>';
                echo '<td>' . $row['nomEpouses'] . '</td>';
                echo '<td>' . $row['prenomEpouses'] . '</td>';
                echo '<td>' . $row['dateNaissanceEpouses'] . '</td>';
                echo '<td>' . $row['dateMariage'] . '</td>';  
                echo '<td>' . $row['numeroActeMariage'] . '</td>';
                echo ' <td><a href="#ajoutPhoto" class="ajoutPhoto"  data-id="'.$row['idEpouses'].'" href="javascript:void(0)" data-toggle="modal"><i class="btn btn-primary" data-toggle="tooltip" title="Edit"><i class="fas fa-camera fa-sm fa-fw mr-2 text-gray-400"></i> </i></a></td>'; 
                echo '<td><a href="update_e_en.php?id='.$row['idEpouses'].'"><i class="btn btn-primary">Modifier</i></a></td>';
              echo '</tr>'; 
            }
            Database::disconnect();  
                ?>
...
<!-- --------------------------------------------------------------------------------------------------------------------------------------- -->
 <div class="row">           
  <div class="modal fade" id="ajoutPhoto" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
  <div class="modal-dialog modal-lg" style="max-width: 1000px!important;"  role="document">
    <div class="modal-content"> 
      <div class="modal-header navbar orange "> 
      <h3 class="labelh">Ajouter ma photo </h3>       
          <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button> 
        </div> 
    <form action="update_db_e_en.php" method="post" enctype="multipart/form-data">  
     
   <div class="modal-body">         
    <label for="file-upload" class="custom-file-upload">
    <i class="fa fa-cloud-upload"></i> Télécharger une photo
</label>
<input type="file" id="imageepouse" name="imageepouse" class="form-control" required/> <br/> 
   </div> 
        <div class="modal-footer row">
          <div class="col-md-5">
          </div>
          <div class="col-md-3">
            <input type="hidden" id="id_epouse" name="id_epouse" value="<?php echo $_GET['id']; ?>">
              <button type="submit" name="photoEpouses" class="btn btn-primary glyphicon glyphicon-save orange1 form-control" > Enregister !</button> 
          </div>
          <div class="col-md-3">
...
<script type="text/javascript">
  ...
  $("#select").on('change', function () {
   ...
   // alert(idpaiement);
   var id_epouse = $(this).attr('data-id'); 
  $("#id_epouse").val(id_epouse);   
