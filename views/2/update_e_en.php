...
$stmt = $pdo->prepare("SELECT * FROM epouses WHERE idEpouses=:idEpouses");
$stmt->execute(array(":idEpouses"=>$_GET['id']));
$userRow=$stmt->fetch(PDO::FETCH_ASSOC);
echo($userRow['idEpouses']);
...
<!-- Ajouter un utilisateur -->
  ...
  <div class="modal-dialog modal-lg" style="max-width: 1800px!important;"  role="document">
    ...
    <div class="etape row">
          ...
          <label class="col-md-3 label1"> Etape 4 / 5</label>  
          <label class="col-md-2 label">Etape 5 / 5</label>    
    </div> 
   <div class="modal-body">         
    <form action="update_db_e_en.php" method="post" class="row">   
    <div class="col-md-12">
    <!-- <label>Photo de l'épouse</label> -->
    <!-- <input type="file" name="image" id="image" class="form-control"  placeholder="Photo de l'épouse" required /> <br/>  -->
      <label>Nom de l'épouse</label>
      <input type="text" name="nomepouses" id="nomepouses" class="form-control"  placeholder="Nom de l'épouse" required value="<?php echo $userRow['nomEpouses']; ?>" /> <br/> 
      <label>Prénom de l'épouse</label>
      <input type="text" name="prenomepouses" id="prenomepouses" class="form-control"  placeholder="Prénom de l'épouse" required  value="<?php echo $userRow['prenomEpouses']; ?>"/><br/> 
    <select name="sex" class="form-control"  value="<?php echo $userRow['sex']; ?>">
      <option value="M">Masculin</option>
      <option value="F">Feminin</option>
    </select><br>
      <label>Date de naissance de l'épouse</label>
      <input type="date" name="datenaissepouses" id="datenaissepouses" class="form-control"  placeholder="Date de naissance de l'épouse" required value="<?php echo $userRow['dateNaissanceEpouses']; ?>"/> <br/> 
      <label>Date de mariage</label>
      <input type="date" name="datemariage" id="datemariage" class="form-control"  placeholder="Date de mariage" required  value="<?php echo $userRow['dateMariage']; ?>"/> <br/> 
      <label>Numéro acte de mariage de l'épouse</label>
      <input type="number" name="numeroactemariage" id="numeroactemariage" class="form-control" value="<?php echo $userRow['numeroActeMariage']; ?>"  placeholder="Numéro acte de mariage  de l'épouse" required /> <br/>  
    </div>  
   </div> 
    <div class="modal-footer row">
            ...
            <div class="col-md-3">
              <input type="hidden" name="idepouse" value="<?php echo $_GET['id']; ?>">
