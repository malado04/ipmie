...
-->
          ...
          <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item dropdown no-arrow" id="content1">
              <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                ...
                <img class="img-profile rounded-circle small" src="http://localhost/ipmies/bootstrap/images/images.jpg">
              </a>
 ...
 <h1 class="alert-danger">Validation des utilisateurs</h1>
...
<tbody>
                        ...
                        $pid=1;
                         ...
                         $statement = $pdo->prepare("SELECT * FROM utilisateur WHERE valide=0 ORDER BY id DESC");  
                        ...
                        echo '<tr>';
                            ...
                            echo '<td>' . $row['tel'] . '</td>'; 
                            echo '<td>' . $row['posteoccupe'] . '</td>';   
                            echo '<td>' . $row['login'] . '</td>';             
                            // echo '<td><a  href="process.php?id='.$row['id'].'"><i class="btn btn-danger">Valider</i></a></td>';                          
                            echo '<td><a class="valider_user" data-id="'.$row['id'].'" href="javascript:void(0)"><i class="btn btn-danger">Valider</i></a></td>';                          
                        echo '</tr>'; 
                        }
                        Database::disconnect();  
                     ?> 
...
<tbody>
                 ...
                 //        $pid=1;
                 //         $pdo->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING ); 
                 //         $statement = $pdo->prepare("SELECT * FROM utilisateur, entreprise WHERE utilisateur.id=entreprise.employeur_id ORDER BY id DESC");  
                 //        $statement->execute(array());  
                 //        $result = $statement->fetchAll();
