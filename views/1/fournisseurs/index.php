...
<body id="page-top">
  ...
  <div id="wrapper">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <h3 class="h">Bienvenue :  <?php echo $userRow['nom']; echo " "; echo $userRow['prenom']; echo " | <i class='fa fa-user fa-fw'></i> "; echo $userRow['posteoccupe']; ?></h3>
 ...
 <div class="d-sm-flex align-items-center justify-content-between mb-4"> 
                    <a href="ajoutFournisseur.php" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm" ><i class="fa fa-user text-white-50"></i> Ajouter un fournisseur</a>
                  </div>        
...
<thead>
            <tr>
              <th><i class="fa fa-id-card " > Matricule</i> </th>
              <th><i class="fa fa-home" > Raison social</i> </th>
              <th><i class="fa fa-user" > Ajouter <br>utilisateur</i> </th>   
...
<tbody>
                        ...
                        $statement = $pdo->prepare("SELECT * FROM fournisseur ORDER BY idfournisseur DESC");  
                        $statement->execute(array());  
                        $result = $statement->fetchAll();
                        foreach ($result as $row) { //on cree les lignes du tableau avec chaque valeur retournée
                        echo '<tr>';
                 //           echo '<td>' . $row['idEmployeur'] . '</td>';
                            echo '<td>' . $row['matriculefournisseur'] . '</td>';
                            echo '<td>' . $row['raisonSocialefournisseur'] . '</td>';
                            // echo '<td>' . $row['siglefournisseur'] . '</td>';

                            echo '<td><a  href="ajoutEmployeur.php?id='.$row['idfournisseur'].'"><i class="btn btn-success"><i class="fa fa-user" ></i></i></a></td>';                            
                            echo '<td><a  href="modifierFournisseur.php?id='.$row['idfournisseur'].'"><i class="btn btn-success"><i class="fa fa-edit" ></i></i></a></td>';                            
                            echo '<td><a href="chois_date.php?id='.$row['idfournisseur'].'"><i class="btn btn-danger"><i class="fa fa-print" ></i></i></a></td>';                          
...
<div class="row">           
  ...
  <!-- End of Page Wrapper --><!-- 
 <?php //include_once 'ajoutfournisseur.php'; ?>
...
<script src="../../../bootstrap/js/bootbox.min.js"></script>
  ...
  // supprimer employers
    $('.trierMois').click(function(e){
   
   e.preventDefault();
   var pid = $(this).attr('data-id'); 
    $('#id_entreprise').val(pid);
