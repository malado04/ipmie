...
<div class="row">
  <h6 class="col-md-4">Raison sociale et adresse de l'employeur : </h6><h6 class="col-md-8"> <?php  echo $userData['raisonSocialefournisseur']; echo " "; ?></h6>
  <h6 class="col-md-4">Mois de cotisation : </h6><h6 class="col-md-8"> <?php  echo $userData['raisonSocialefournisseur']; echo " "; ?></h6>
...
<table id="border" class="left padding" style="width:100%">
  <tr> 
    ...
    <td class="prenom center"><h4>Prise en <br>charge <br> participant</h4></td>
     <?php 
    $statement = $pdo->prepare("SELECT *, SUM(montant)AS montantsum, COUNT(idEmployer) AS nombre FROM fournisseur, entreprise, employer, bondecommande WHERE entreprise.idEntreprise=employer.employeur_id AND bondecommande.id_parent_p=employer.codeemployer  AND idfournisseur=:id GROUP by idEntreprise");  
    $statement->execute(array(":id"=>$_GET['id']));  
