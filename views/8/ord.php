...
<div class="" id="id_consultation" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <form action="ord1.php" method="post">
      ...
      <div class="col-md-12">   
           <legend>Choix des maladie </legend>
           <select id="maladie" name="maladie[]" multiple="multiple" class="form-control" required="">
            <?php  $pdo =Database::connect();
                         $pdo->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING ); 
                $stmt1 = $pdo->prepare("SELECT * FROM item_med, employer, consultation WHERE employer.idEmployer=item_med.med_idEm AND consultation.idcons=item_med.med_idc AND employer.codeemployer=:id");
                $stmt1->execute(array(":id"=>$_POST['id']));                        
                        $result = $stmt1->fetchAll();
                        foreach ($result as $row) {  
                      ?>
                        <option class="option" value="<?php echo $row['med_item_con']; ?>"><?php echo $row['med_item_con']; echo " "; echo $row['created_med'];?></option> 
                      <?php 
                         }
                        Database::disconnect();  
                     ?> 
          ...
          <div class="modal-footer bg-primary text-white">
            <input type="hidden" class="form-control" name="codeben" id="codeben" value="<?php echo $_POST['id'];?>">
