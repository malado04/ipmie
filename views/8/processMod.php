<?php 


   if (isset($_POST['modifierProfil'])) {

   $userData = array(
   'cni'=> $_POST['cniemployeurmod'],
   'nom'=> $_POST['nomemployeurmod'],
   'prenom'=> $_POST['prenomemployeurmod'],
   'telFixe'=> $_POST['telfixemployeurmod'],
   'telPortable'=> $_POST['telportemployeurmod'],
   'adresseCommune'=> $_POST['communemod'],
   'adresse'=> $_POST['quartiermod'],
   'email'=> $_POST['emailmod'],
   'login'=> $_POST['loginmod'],
   'password'=> $_POST['passwordmod'] 
   );

   $condition = array('id' => $_POST['iduser']);
  require_once 'DB.php';
   $db = new DB();
   $tblName = 'utilisateur';
   $update = $db->update($tblName,$userData,$condition);
     // var_dump($userData); 
     header('Location: success.php'); 
 }

// -------------------------------------------------Modifier la photo--------------------------------------- 

  if (isset($_POST['modifierPhoto'])) {

  $imgFile = $_FILES['image']['name'];
  $tmp_dir = $_FILES['image']['tmp_name'];
  $imgSize = $_FILES['image']['size'];
  $temp  = $_FILES["image"]["tmp_name"];
  $imgFilename=$imgFile+$_POST['cniemployeurmod'];
  $upload_dir ="../../img/Profil/".$imgFilename; //set upload folder path
    move_uploaded_file($tmp_dir, "../../img/Profil/" .$imgFilename); //move upload file temperory directory to your upload folder
     // move_uploaded_file($tmp_dir,$upload_dir.$userpic);


   $userData = array( 
   'image'=> $imgFilename
   );

   $condition = array('id' => $_POST['iduser']);
  require_once 'DB.php';
   $db = new DB();
   $tblName = 'utilisateur';
   $update = $db->update($tblName,$userData,$condition);
     var_dump($userData); 
     header('Location: success.php'); 
 }

  ?>