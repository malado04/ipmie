...
<h2 class="text-center">Liste des employers</h2><br><br>
        <!-- Begin Page Content -->
        <div class="container-fluid">
        ...
        <table id="paiement" class="table table-hover table-striped table-bordered" style="width:100%">
          ...
          <tbody>
            ...
            foreach ($result as $row) { 
              echo '<tr>';
                echo '<td>' . $row['idEmployer'] . '</td>';
                echo '<td><img  class="width_img img-profile" src="../../img/employer/'.$row['image'].'"></td>';
                echo '<td>' . $row['codeemployer'] . '</td>';
                echo '<td>' . $row['cniemployer'] . '</td>';
                echo '<td>' . $row['nomEmployer'] . '</td>';
                echo '<td>' . $row['prenomEmployer'] . '</td>'; 
                echo ' <td><a href="#ajoutPhoto" class="ajoutPhoto"  data-id="'.$row['idEmployer'].'" href="javascript:void(0)" data-toggle="modal"><i class="btn btn-primary" data-toggle="tooltip" title="Edit"><i class="fas fa-camera fa-sm fa-fw mr-2 text-gray-400"></i></i></a></td>'; 
              echo '</tr>'; 
            }
            Database::disconnect();  
                ?>
...
<input type="file" id="imageemployer" name="imageemployer" class="form-control" required/> <br/> 
   </div> 
        <div class="modal-footer row">
          <div class="col-md-5">
          </div>
          <div class="col-md-3">
            <input type="hidden" id="id_Employer" name="id_Employer">
              <button type="submit" name="photoEmployer" class="btn btn-primary glyphicon glyphicon-save orange1 form-control" > Enregister !</button> 
          </div>
          <div class="col-md-3">
...
<script type="text/javascript">
  ...
  $("#select").on('change', function () {
   ...
   // alert(idpaiement);
   var id_Employer = $(this).attr('data-id'); 
  $("#id_Employer").val(id_Employer);   
