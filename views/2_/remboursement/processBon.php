...
$stmt = $pdo->prepare("SELECT * FROM fournisseur WHERE matriculefournisseur=:idfournisseur");
$stmt->execute(array(":idfournisseur"=>$_POST['idstructure']));
$entrepriseRow=$stmt->fetch(PDO::FETCH_ASSOC);
$count=count($entrepriseRow);

if (isset($_POST['medicament1']) && isset($_POST['prixmedi1']) && (!empty($_POST['medicament1']))) {
