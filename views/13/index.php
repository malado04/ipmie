...
$stmt->execute(array(":id"=>$_SESSION['user_session_remboursement'], ":valide"=>$valide));
$userRow=$stmt->fetch(PDO::FETCH_ASSOC);

if(!isset($_SESSION['user_session_remboursement']))
{
  header("Location: ../../");
}
...
<table id="listeEmployert" class="table table-hover table-striped table-bordered" style="width:100%">
  ...
  <tbody>
    ...
    $statement = $pdo->prepare("SELECT * FROM entreprise, employer WHERE employer.employeur_id= entreprise.idEntreprise ORDER BY idEntreprise DESC");  
    ...
    foreach ($result as $row) { //on cree les lignes du tableau avec chaque valeur retournée
      ...
      echo ' <td><a href="#ajoutRemboursement" class="ajoutRemboursement"  data-id="'.$row['idEntreprise'].'" href="javascript:void(0)" data-toggle="modal"><i class="btn btn-primary" data-toggle="tooltip" title="Edit">Ajouter</i></a></td>';
      echo ' <td><a href="print.php?id_bon='.$row['idEmployer'].'" target="_blank" ><i class="btn btn-success" data-toggle="tooltip" title="Edit">Remboursement</i></a></td>';                            
      echo '</tr>'; 
      }
      Database::disconnect();  
    ?> 
...
<script type="text/javascript" src="script.js"></script>  
  <script type="text/javascript">
    ...
    $('.ajoutRemboursement').click(function(e){
     e.preventDefault();
