...
// *******************************************************************************************************************************************
 ...
 $stmt2 = $pdo->prepare("SELECT idEntreprise, matriculeEntreprise FROM entreprise WHERE matriculeEntreprise=:mat"); 
                  $stmt2->execute(array(":mat" =>$_SESSION['session_idEmployer']));
                  $row2 = $stmt2->fetch(PDO::FETCH_ASSOC);  
                  $idEntreprise = $row2['idEntreprise'];
   } 
         include 'DB.php';
         $db = new DB();
         $tblName = 'entreprise';
         $userData = array(
            ...
            'employeur_id'=>$idEntreprise            
        );
         var_dump($userData);
            $insert = $db->insert($tblName,$userData);
               if($insert)
               {   
                  $_SESSION['session_idEntreprise'] = $idEntreprise;
...
//*********************************************************************************************************************************************
 ...
 {  
   ...
   // $stmt2->execute(array());
   // $row2 = $stmt2->fetch(PDO::FETCH_ASSOC); 
   // $codeemployer= $idEmployeur.$row1['codeemployer']+1;  
...
$userData = array(
               ...
               'montant'=>$salairep,
               'employeur_id'=>$_SESSION['session_idEntreprise']
        );
var_dump($userData);
   //      $insert = $db->insert($tblName,$userData);
   //             if($insert)
   ...
   if ($row0>0) {
	...
	var_dump($userData);
        ...
        $insert = $db->insert($tblName,$userData);
               ...
               if($insert)
               {   
                  header('Location: https://ipmiez.com/views/1/'); 
               }else{
                  echo "requete"; 
   }
